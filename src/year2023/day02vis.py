...
MAX_CUBES = (
    max(int(i) for line in lines for i in re.findall("\d+", line.split(": ")[1])) + 1
)
...
area_game = screen.subsurface(pg.Rect(0, TOTALS_HEIGHT, WIDTH, HEIGHT - TOTALS_HEIGHT))
...
while running:
    ...
    for i, game_round in enumerate(game.rounds):
        ...
        for c, color in enumerate(LIMIT.keys()):
            ...
            if count > LIMIT[color] and count - 1 < dt * MAX_CUBES / CUBES_APPEAR_TIME:
            ...
            if (
                ...
                rect = label.get_rect(
                    ...
                    top=TOTALS_HEIGHT + PADDING + CUBE_HEIGHT * (MAX_CUBES - count - 1),
                )
    ...
    if dt > CUBES_APPEAR_TIME and dt < TIME_PER_TURN:
        moved_degree = min(
            1, max(0, (dt - CUBES_APPEAR_TIME) / (TIME_PER_TURN - CUBES_APPEAR_TIME))
        )
        if game.score_1 > 0:
            # render flying label 1
            ...
            rect_2.left = rect_2.left * moved_degree + rect_1.left * (1 - moved_degree)
